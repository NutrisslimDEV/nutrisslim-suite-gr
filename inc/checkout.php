...
// NEW UPDATE CUSTOM CHECKOUT BLOCK
function update_custom_checkout_block() {
    ...
    error_log('Free shipping session: ' . $free_shipping_session);
    error_log('LID----------:' . $lid);
    // Defensive fallback: if not on a landing page, remove session
    if ($free_shipping_session != 1) {
        WC()->session->__unset('free_shipping');
        WC()->session->__unset('chosen_shipping_methods');
        WC()->shipping->reset_shipping();
        WC()->cart->calculate_shipping();
        WC()->cart->calculate_totals();
        error_log('Cleared forced free shipping because user left landing page');
    }
    ...
    if ($force_free_shipping) {
        foreach ($available_methods as $method_id => $method) {
            ...
            // Check if this method is an actual free_shipping
            if (strpos($method_id, 'free_shipping') !== false) {
                ...
                // Keep only free_shipping as available
                $available_methods = [$method_id => $method];
        ...
        // If not forcing, detect if any free shipping is naturally available
        foreach ($available_methods as $method_id => $method) {
            if (strpos($method_id, 'free_shipping') !== false) {
                $has_free_shipping = true;
                $free_shipping_method = $method;
    ...
    // Enrich shipping methods with cost, fee, tax
    foreach ($available_methods as $method_id => $method) {
        $instance_settings = get_option('woocommerce_flat_rate_' . $method->instance_id . '_settings') ?: [];

        $raw_cost = $instance_settings['cost'] ?? '0';
        $cost = floatval(str_replace(',', '.', $raw_cost));

        $raw_fee = $instance_settings['part_of_fee'] ?? '0';
        $fee = floatval(str_replace(',', '.', $raw_fee));
...
//            intval($instance_settings['delivery_from'] ?? 2),
//            intval($instance_settings['delivery_to'] ?? 3)
//        );

        $output .= sprintf(
            '<div class="rf-option-container" style="background-color:rgb(176, 228, 197);">
                <div class="rf-option-left">
                    <input data-freelabel="%s" data-label="%s" type="radio" class="rf-custom-checkbox radio"
                        name="rf_nacin_dostave" id="rf_%s" value="%s" checked="checked">
                    <label for="rf_%s">
            ...
            esc_html($free_shipping_method->label . $type),
            esc_attr($method_id),
            esc_attr($method_id),
            esc_attr($method_id),
            esc_html($free_shipping_method->label . $type),
            esc_html(__('Gratis', 'woocommerce')),
//            esc_html($delivery_days['from'] . ' - ' . $delivery_days['to'])
        );
...
add_filter('woocommerce_shipping_free_shipping_is_available', function ($is_available, $package, $shipping_method) {
    $force_free_shipping = (WC()->session->get('free_shipping') == 1);
...
//                        <div class="rf-option-title">' . esc_html($free_shipping_method->label) . $type . ' (' . esc_attr(__('Δωρεάν', 'nutrisslim-suiteV2')) . ')</div>
