...
function custom_get_additional_fees_settings() {
    ...
    $additional_fees_settings = array(
        ...
        'priority_delivery' => array(
            'name' => __('Αποστολή με προτεραιότητα', 'nutrisslim-suiteV2'),
            'type' => 'number',
        ...
        'package_insurance' => array(
            'name' => __('Ασφάλιση δέματος', 'nutrisslim-suiteV2'),
            'type' => 'number',
...
function add_custom_fees_to_cart() {
    ...
    $custom_fees = [
        ...
        'rf_zavarovanje_narocila' => [
            'label' => __('Ασφάλιση δέματος', 'nutrisslim-suiteV2'),
        ...
        'rf_priority_delivery' => [
            'label' => __('Αποστολή με προτεραιότητα', 'nutrisslim-suiteV2'),
...
<div id="delivery-option" class="rf-custom-option">
    <h3><?php echo __('Αναβαθμίστε την αποστολή σας', 'nutrisslim-suiteV2'); ?></h3>
    ...
    <div class="delivery-option rf-option-container" style="<?php echo $is_priority_delivery_bg; ?>">
        <div class="rf-option-left">
            ...
            <label for="rf_priority_delivery">
                <div class="rf-option-title"><?php echo __('Παράκαμψη της ουράς', 'nutrisslim-suiteV2'); ?>
                    (+<?php echo wc_price($priority_delivery); ?>)</div>
                <div class="rf-option-description">
                    <?php echo __('Επιλέξτε συσκευασία με προτεραιότητα για την ταχύτερη αποστολή. Επιλέγοντας αυτήν την επιλογή, η παραγγελία σας θα επεξεργαστεί πριν από άλλες.', 'nutrisslim-suiteV2'); ?>
                </div>
            </label>
        </div>
    ...
    <div class="delivery-option rf-option-container" style="<?php echo $is_checked_zavarovanje_bg; ?>">
        <div class="rf-option-left">
            ...
            <label for="rf_zavarovanje_narocila">
                <div class="rf-option-title"><?php echo __('Ασφάλιση δέματος', 'nutrisslim-suiteV2'); ?>
                    (+<?php echo wc_price($package_insurance); ?>)</div>
                <div class="rf-option-description">
                    <?php echo __('Διασφαλίστε την αποστολή του δέματος και επωφεληθείτε από μια εγγυημένη και δωρεάν λύση σε περίπτωση κατεστραμμένων ή χαμένων προϊόντων. Θα αποστείλουμε το δέμα ξανά χωρίς κανένα επιπλέον κόστος για εσάς.', 'nutrisslim-suiteV2'); ?>
                </div>
            </label>
        </div>
...
<div class="rf-custom-option select-shipping">
    <h3><?php echo __('Αποστολή', 'woocommmerce'); ?></h3>
...
function custom_woocommerce_loop_add_to_cart_link($html, $product) {
    ...
    $custom_text = __( 'Προσθήκη στο καλάθι', 'nutrisslim-suiteV2' );
...
function cart_upsell() {
    ...
    if (!empty($items)) {
        ...
        $output .= '<h3>' . __('Μην χάσετε τις καλύτερες προσφορές!', 'nutrisslim-suiteV2') . '</h3>';
        $output .= '<p>' . __('Μόνο για περιορισμένο χρονικό διάστημα.', 'nutrisslim-suiteV2') . '</p>';
        ...
        foreach ($items as $item) {
            ...
            $output .= '<button class="cart-button_swiper add-to-cart-icon" data-offer="cart_upsell" data-product-id="' . $pid . '" data-quantity="1" data-price="' . $offer_price . '" ' . $disabled . '>' . __('Προσθήκη στο καλάθι', 'nutrisslim-suiteV2') . '</button>';
...
function add_meta_to_order_fees($item, $fee_key, $fee, $order) {
    ...
    $fee_sifrant_map = array(
        ...
        'αποστολή-με-προτεραιότητα' => get_option('priority_delivery_fee_sifrant'),
        'eco-friendly-packaging' => get_option('custom_eco_friendly_packaging_sifrant'),
        'ασφάλιση-δέματος' => get_option('custom_package_insurance_sifrant'),
        'πρόσθετη-χρέωση-για-πληρωμή-με-αντικαταβολή' => 'STRL',
    );

    // Decode any URL encoding from the fee key
    $decoded_fee_key = urldecode($fee_key);
    
    // Debug: log the decoded fee key to your PHP error log
    error_log('Decoded fee key: ' . $decoded_fee_key);

    // Normalize the fee key to match the map (convert to lowercase and replace spaces with dashes)
    $normalized_fee_key = strtolower(str_replace(' ', '-', $decoded_fee_key));

    // Debug: log the normalized fee key to your PHP error log
    error_log('Normalized fee key: ' . $normalized_fee_key);

    // Check if the normalized fee key exists in the map and add corresponding sifrant as meta
    $matched = false;
foreach ($fee_sifrant_map as $key => $sifrant_value) {
    if ((str_starts_with($key, $normalized_fee_key) || str_starts_with($normalized_fee_key, $key)) && !empty($sifrant_value)) {
...
if (!$matched) {
    error_log('Fee key not found in map: ' . $normalized_fee_key);
}
